...
# BMI-capable PENBMI and nonlinear PENNON libraries may be provided with the
# `PENOPT_LIBPENBMI` and `PENOPT_LIBPENNON` environment variables. A path may
# point either to a shared library, which is used as is, or to the static
# archive shipped by PENOPT, in which case a shared library is linked here.
...
# Julia can only `ccall` into a shared library, so PENOPT static archives are
# relinked here. `entrypoint` avoids force-loading optional objects that depend
# on interfaces, such as AMPL, that are not used by this wrapper.
function _build_penopt_library(archive, name; entrypoint = nothing)
    mkpath(LIB_DIR)
    output = joinpath(LIB_DIR, "lib$(name).$(Libdl.dlext)")
    julia_libdir = joinpath(Sys.BINDIR, Base.LIBDIR, "julia")
    os, _ = _platform()
    if os == :linux
        ...
        archive_args = entrypoint === nothing ?
            `-Wl,--whole-archive $(archive) -Wl,--no-whole-archive` :
            `-Wl,-u,$(entrypoint) $(archive)`
        cmd = `gcc -shared -fPIC -o $(output)
                   -Wl,--no-undefined $(archive_args)
                   -L$(julia_libdir) -Wl,-rpath,$(julia_libdir)
                   $(libopenblas) -Wl,-rpath,$(dirname(libopenblas))
                   -lgfortran -lpthread -ldl -lm`
    elseif os == :apple
        archive_args = entrypoint === nothing ?
            `-Wl,-force_load,$(archive)` : `-Wl,-u,_$(entrypoint) $(archive)`
        cmd = `gcc -dynamiclib -o $(output)
                   $(archive_args)
                   -framework Accelerate
                   -L$(julia_libdir) -Wl,-rpath,$(julia_libdir)
        ...
        error(
            ...
            "`lib$(name).dll` manually, point the corresponding PENOPT " *
            "environment variable at it, and re-run `Pkg.build(\"Penopt\")`.",
        )
    end
    @info "Linking shared library" cmd
    run(cmd)
...
_build_libpenbmi(archive) = _build_penopt_library(archive, "penbmi")
...
function _libpennon_path()
    path = get(ENV, "PENOPT_LIBPENNON", "")
    if isempty(path)
        return path
    elseif !isfile(path)
        error(
            "The `PENOPT_LIBPENNON` environment variable is set to " *
            "`$(path)` but no such file exists.",
        )
    end
    return _is_static_library(path) ?
           _build_penopt_library(path, "pennon"; entrypoint = "pennlp") : path
end

function _write_deps(libpensdp_path, libpenbmi_path, libpennon_path)
    open(joinpath(DEPS_DIR, "deps.jl"), "w") do io
        ...
        println(io, "const libpennon = ", repr(libpennon_path))
        println(io)
        println(io, """
            function check_deps()
                if !isfile(libpensdp)
                ...
                if !isempty(libpennon)
                    if !isfile(libpennon)
                        error("\$(libpennon) does not exist.")
                    end
                    if Libdl.dlopen_e(libpennon) == C_NULL
                        error("\$(libpennon) cannot be opened.")
                    end
                end
                return
            end""")
